...
3. MCP 始终需要独立 Token（`mcpTokenRequired: true`）。缺 Token / `mcp_token_required` → 请用户到「设置 → 安全」**重新复制 MCP 配置**，不要把 CLI 可选令牌当成 MCP Token。仅当 `cliTokenRequired` / 兼容字段 `tokenRequired` 为 true 时，CLI HTTP / WebSocket 才需要 CLI 令牌（复用已记 token，否则问用户）；禁止猜 / 翻本地
