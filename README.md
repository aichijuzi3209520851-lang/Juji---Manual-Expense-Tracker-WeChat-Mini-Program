...
- **小橘账单查询分析**：支持“这周花了多少钱 / 这周比上周省了多少 / 本月哪类花得最多”等高频问题，云函数直接查询 `bills` 并确定性计算金额，不让 AI 编数字
...
| `aiChat` | Nodejs16.13 | 「小橘」聊天助手 + 账单查询分析 + 对话记账解析（确定性查库统计 + 混元 `generateText` 结构化 JSON） |
